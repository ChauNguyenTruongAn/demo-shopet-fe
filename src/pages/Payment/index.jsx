...
const Payment = () => {
    ...
    const validateOrder = () => {
        ...
        // if (hasOutOfStock) {
        //     setValidationError('Có sản phẩm hết hàng trong giỏ. Vui lòng xóa để tiếp tục');
        //     return false;
        // }
        ...
        try {
            ...
            if (selectMethodPay === 'VNPAY') {
                if (response.data.status === 200) {
                    const linkResponse = await orderApis.getPaymentLink(
                        total + deliveryFee,
                        response.data.data.orderId,
                    );
                    window.location.href = linkResponse.data;
                }
            } else {
                setIsShowNotificationSuccess(true);
                setCart([]);
